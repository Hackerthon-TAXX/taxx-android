...
class ServicestartActivity :
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val Data2 = RiderLocationData(
            postFormData.ridersId,
            postFormData.arrivalLatitue,
            postFormData.arrivalLongitude,
            0,
            "test"
        )
    ...
    private fun getMovingRider(data: RiderLocationData) {
        RetrofitInterface().getInstance().create(GetmovingAPI::class.java)
            .getRidersLocation(data.id, data.latitude, data.longitude)
            .enqueue(object : Callback<GetMovingResponse> {
        ...
        Thread {
            ...
            while (distance > 500) {
                ...
                runOnUiThread {
                    ...
                    val percent =
                        ((startDistance.toDouble() - distance.toDouble()) / startDistance.toDouble()) * 50
        ...
        Thread {
            ...
            while (distance > 500) {
                ...
                runOnUiThread {
                    binding.tvDistance.text = text
                    val percent =
                        ((startDistance.toDouble() - distance.toDouble()) / startDistance.toDouble()) * 50 + 50
                    bar.progress = percent.toInt()
                }
                Thread.sleep(500)
            }
